...
export function DirectorPanel({ projectId, frames }: { projectId: string; frames: readonly StudioAssetView[] }) {
  ...
  return <section className={styles.directorGrid}>
    ...
    <aside className={styles.vlmPanel}><div className={styles.panelHeader}><div><span>GEMINI · REAL API</span><h2>VLM 导演</h2></div></div><p>按 A、B 顺序分析当前真实图片，并结合你的意图生成视频镜头提示词。</p><div className={styles.vlmInputs}>{frames.map((frame, index) => <div key={frame.id}><Image alt={frame.displayName} height={72} src={frame.contentUrl} unoptimized width={128}/><span>{index === 0 ? "A" : index === 1 ? "B" : `R${index - 1}`}</span></div>)}{frames.length === 0 ? <div className={styles.miniEmpty}>没有图片也可做纯文本导演</div> : null}</div><button className={styles.primaryButtonWide} disabled={busy || !intent.trim()} onClick={() => void generate()} type="button">{busy ? "正在分析…" : "确认并生成导演建议"}</button><small aria-live="polite" className={styles.honestNote}>{message || "调用前会再次确认真实费用。"}</small></aside>
  </section>;
}
