...
export function DirectorPanel({ projectId, frames }: { projectId: string; frames: readonly StudioAssetView[] }) {
  ...
  return <section className={styles.directorGrid}>
    ...
    <AiActionDialog confirmLabel="确认并开始生成" cost="价格待账单确认" description="Gemini VLM 将读取当前镜头意图和参考画面，生成适配目标视频模型的提示词。" facts={[targetModel, `${duration} 秒`, audio ? "包含音频提示" : "纯视觉提示"]} onClose={() => { if (!busy) setDialogOpen(false); }} onConfirm={() => void generate()} open={dialogOpen} phase={dialogPhase} progressLabel="正在生成视频导演提示词" statusMessage={dialogMessage} title="生成视频导演提示词"/>
  </section>;
}
