...
function ReviewDialog({ open, capability, values, bindings, projectId, onClose, returnRef }: { open: boolean; capability: CapabilityView; values: ParameterValues; bindings: Record<string, string[]>; projectId: string; onClose: () => void; returnRef: React.RefObject<HTMLButtonElement | null> }) {
  ...
  if (!open) return null;
  function keyDown(event: ReactKeyboardEvent<HTMLDivElement>) { if (event.key === "Escape" && !submitting) { event.preventDefault(); onClose(); } }
  ...
  return (
    <div className={styles.dialogLayer}><button aria-label="关闭复核弹层" className={styles.dialogBackdrop} disabled={submitting} onClick={onClose} type="button"/><div aria-describedby="review-description" aria-labelledby="review-title" aria-modal="true" className={styles.dialog} onKeyDown={keyDown} role="dialog"><div className={styles.dialogHeader}><div><span>提交前检查</span><h2 id="review-title">生成配置复核</h2></div><button aria-label="关闭复核" disabled={submitting} onClick={onClose} ref={closeRef} type="button">×</button></div><p id="review-description">确认后会立即调用真实模型并产生对应费用；异步 task ID 会先保存，再进入查询。</p><dl><div><dt>Provider</dt><dd>{providerLabel(providerId(capability))}</dd></div><div><dt>模型</dt><dd>{capability.modelLabel}</dd></div><div><dt>模式</dt><dd>{capability.modeLabel}</dd></div><div><dt>输入</dt><dd>{capability.inputSlots.map((slot) => `${slot.label} ${bindings[slot.id]?.length ?? 0} 项`).join(" + ") || "纯文本"}</dd></div><div><dt>时长</dt><dd>{String(values.duration)} 秒</dd></div><div><dt>分辨率</dt><dd>{String(values.resolution ?? values.modelMode ?? "由模型决定")}</dd></div></dl><div className={styles.dialogNotice}><StudioIcon name="settings" size={17}/><span><strong>真实付费请求</strong><small>{capability.pricing.kind === "exact" ? `预计 ¥${capability.pricing.amountCny.toFixed(2)}` : capability.pricing.kind === "range" ? `预计 ¥${capability.pricing.minCny.toFixed(2)}–¥${capability.pricing.maxCny.toFixed(2)}` : "当前文档无法精确估价"}</small></span></div>{submitting ? <div className={styles.submissionProgress}><div aria-label="AI 任务提交进度" role="progressbar"><i/></div><strong>正在向视频模型提交任务…</strong><small>任务 ID 保存成功后才会进入队列。</small></div> : message ? <p aria-live="polite">{message}</p> : null}<div className={styles.dialogActions}>{result ? <Link href={`/projects/${encodeURIComponent(projectId)}/jobs`}>查看任务</Link> : <><button disabled={submitting} onClick={onClose} type="button">返回调整</button><button disabled={submitting} onClick={() => void submit()} type="button">{submitting ? "正在提交…" : "确认费用并提交"}</button></>}</div></div></div>
  );
}
